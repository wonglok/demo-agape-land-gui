...
void main() {
    ...
    gl_FragColor = vec4(colorPicker, opacity);
}
`;
    let defaultPingPong = `
    //
...
uniform vec3 mouse;
 ...
 // Originally sourced from https://www.shadertoy.com/view/ldfSWs
      ...
      float doModel(vec3 p) {
        ...
        vec3 pp = opTwist(p, sin(myTimer));

        float ten = sdOctahedron(pp, 4.0);

        float star = opRound(ten, 0.1);
...
swing.y = cos(time * 60.0);
  ...
  lastPos.xyz = rotateQ(normalize(vec3(0.0, 0.0, 1.0)), time * 1.0) * nPosition.xyz;
...
`;
    ...
    this.core.onLoop(() => {
      pingMat.uniforms.mouse.value.copy({
        x: Core.now.canvas.now.mouse.x,
        y: Core.now.canvas.now.mouse.y,
        z: 0,
      });
