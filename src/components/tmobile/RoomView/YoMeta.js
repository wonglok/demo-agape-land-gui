...
// import { ColliderCache } from "../BirdView/collider/ColliderCache.js";
import { ColliderCache } from "./collider/ColliderCache.js";

// const WeakMapYo = new WeakMap();
export class YoMeta extends Object3D {
  constructor({ camera, scene, initY = 0, controls }) {
    super();
    ...
    this.reset = () => {
      ...
      this.player.position.set(0, initY, 0);
      this.controls.target.set(0, initY, 0);
      this.camera.position.set(0, initY, 0.1);
    ...
    this.cacheMap = ColliderCache;
    this.parseScene = async ({ scene }) => {
      if (this.cacheMap.has(scene.uuid)) {
        return this.cacheMap.get(scene.uuid).then((collider) => {
          this.collider = collider;
        });
      }

      let prom = sceneToCollider({ scene });
      this.cacheMap.set(scene.uuid, prom);
    ...
    this.update = () => {
      ...
      if (this.useKeyboard && this.params.fwdPressed) {
      ...
      if (this.useKeyboard && this.params.bkdPressed) {
        this.tempVector.set(0, 0, 1).applyAxisAngle(this.upVector, angle);
        this.player.position.addScaledVector(
          this.tempVector,
          this.params.playerSpeed * delta
      ...
      if (this.useKeyboard && this.params.lftPressed) {
      ...
      if (this.useKeyboard && this.params.rgtPressed) {
        this.tempVector.set(1, 0, 0).applyAxisAngle(this.upVector, angle);
        this.player.position.addScaledVector(
          this.tempVector,
          this.params.playerSpeed * delta
      ...
      if (this.useKeyboard && this.keyState.joyStickDown) {
        this.tempVector
          .set(0, 0, -1)
      ...
      // adjust the camera
      // this.player.position.y += offsetY * 0.0;
      this.camera.position.sub(this.controls.target);
      this.controls.target.copy(this.player.position);
      this.camera.position.add(this.player.position);
      // this.player.position.y -= offsetY * 0.0;
