  ...
  constructor({ array, mini }) {
    ...
    let headerCode = /* glsl */ `
        ...
        void computeSphere (float index, inout vec3 position, inout vec3 velocity) {
          ...
          velocity += jade(vec3(position.rgb * 1.0)) / 1.0 * 0.3 * 0.5 * min(len, 0.2) * 4.0 * 2.5;
...
export class PositionSimulation {
  constructor({ mini, howManyTracker, influences = DefaultInfluence, renderer, cursorPointer }) {
    ...
    this.commonHeaders = () => {
      return `
        ...
        bool detectReset (vec3 position, vec4 pos, vec4 vel) {
          return length(position) >= 3.25 || pos.w >= 0.99;
        }
      `
    }
