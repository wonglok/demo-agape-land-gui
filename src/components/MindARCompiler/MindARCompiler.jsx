...
export function MindARCompiler() {
  ...
  let compile = ({ fileURL, autoStart = false }) => {
    ...
    Promise.all([
      ...
      async ([
        ...
        rgbe.loadAsync(`/thankyou-jesus/evening_road_01_puresky_1k.hdr`).then((tex) => {
  ...
  useEffect(() => {
    compile({ fileURL: `/thankyou-jesus/thankyouJESUS.jpg`, autoStart: false })
  }, [])
    ...
    <>
      ...
      <div className='absolute left-0 top-0 flex h-full w-full flex-col items-center justify-center'>
        {!noGUI && start && (
          <>
            <button
              className='m-1 rounded-2xl bg-blue-300 p-2'
              onClick={() => {
                // start()
            ...
            {progress}
            <img className='w-40' src={`/thankyou-jesus/thankyouJESUS.jpg`}></img>
          </>
        )}
      </div>
    </>
